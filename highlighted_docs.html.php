...
<div id="main">
	...
	<div id="left_col">
		...
		while($row = $res_ev->fetch_assoc()){
			if(isset($row['privato']) && $row['privato'] == 1){
				if(!($row['permessi']&$_SESSION['__user__']->getPerms())) break;
			}
			$path_parts = pathinfo($row['file']);
        ?>
			<a href="download_manager.php?doc=document&id=<?php print $row['id'] ?>" style="font-weight: normal; text-decoration: none">
				<div class="card">
					<div class="card_title">
						<?php echo truncateString($row['titolo'], 100) ?>
					</div>
					<div class="card_minicontent">
						<?php echo $row['commento'] ?>
						<div style="float: right; margin-right: 20px; width: 100px">
							Tipo file: <?php echo strtoupper($path_parts['extension']) ?>
						</div>
					</div>
				</div>
			</a>
